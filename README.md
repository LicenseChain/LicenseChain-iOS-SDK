...
- **⚡ High Performance** - Optimized for iOS runtime
...
**RS256 + JWKS:** Verify with **`LicenseChainLicenseAssertion.verifyLicenseAssertionJwt`** (JOSESwift). Resolve signing keys via **`GET /v1/licenses/jwks`** using the JWT **`kid`** (or the **`license_jwks_uri`** returned with the token).

**iOS:** In-process verification is supported when JOSESwift is linked per this SDK. See [THIN_CLIENT_PARITY](https://docs.licensechain.app/).
...
1. Download the latest release from [GitHub Releases](https://github.com/LicenseChain/LicenseChain-iOS-SDK/releases)
...
- [LicenseChain macOS SDK](https://github.com/LicenseChain/LicenseChain-macOS-SDK)
---
