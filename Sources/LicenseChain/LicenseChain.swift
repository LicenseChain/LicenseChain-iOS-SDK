...
public class LicenseChain {
    ...
    private func makeRequest<T: Codable>(method: String, endpoint: String, body: Any? = nil, queryParams: [String: Any]? = nil) async throws -> T {
        ...
        request.setValue("ios-sdk", forHTTPHeaderField: "X-Platform")
        request.setValue("LicenseChain-iOS-SDK/1.0.0", forHTTPHeaderField: "User-Agent")
    ...
    private func defaultHWUID() -> String {
        ...
        if let vendor = UIDevice.current.identifierForVendor?.uuidString, !vendor.isEmpty {
            return sha256("licensechain|ios|\(vendor)|\(UIDevice.current.systemName)|\(UIDevice.current.systemVersion)")
        }
        #endif
        let raw = "licensechain|ios|\(ProcessInfo.processInfo.hostName)|\(ProcessInfo.processInfo.operatingSystemVersionString)|unknown"
        return sha256(raw)
    }
